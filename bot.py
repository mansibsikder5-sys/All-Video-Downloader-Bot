...
@app.route('/')
def home():
    return "⚡ Downloader Bot with Original Modified Engine is Running! ⚡"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🛠️ আপনার প্রথম কোডের অরিজিনাল ডাউনলোড মেথড (মডিফাইড হেডারসহ)
    ydl_opts = {
        # প্রথম কোডের মতো বেস্ট কোয়ালিটি ফরম্যাট যা ৫০ এমবির নিচে থাকবে
        'format': 'bestvideo[filesize<=45M]+bestaudio/best[filesize<=45M]/best', 
        'outtmpl': 'downloaded_video.%(ext)s', 
        'quiet': True,
        'no_warnings': True,
        
        # ইনস্টাগ্রাম ও ইউটিউবের ব্লকিং এড়াতে আসল ব্রাউজার এমুলেশন
        'extractor_args': {
            'youtube': {
                'player_client': ['android', 'web'], # মোবাইল ফোনের মতো সিগন্যাল পাঠাবে
                'skip': ['dash', 'hls']
            },
            'instagram': {
                'max_comments': 0,
        ...
        # FastDL স্টাইলের প্রিমিয়াম ইউজার-এজেন্ট হেডার
        'http_headers': {
    ...
    try:
        ...
        if filesize > 50.0:
            await status_message.edit_text(
                f"🛑 দুঃখিত! ভিডিওটির সাইজ ({filesize:.1f} MB) ফ্রি বটের লিমিটের (50 MB) চেয়ে বেশি বড়।\n\n"
                "💡 এটি ছোট রিলস, শর্টস বা কম সময়ের ভিডিওর ক্ষেত্রে দারুণ কাজ করবে!"
            )
        ...
        await status_message.edit_text(
            ...
            "২. ভিডিওটির সাইজ কোয়ালিটি কমানোর পরেও ৫০ মেগাবাইটের বেশি। 🛑\n"
            "৩. ইনস্টাগ্রাম/ইউটিউব আমাদের হোস্টিং সার্ভার আইপি সাময়িক ব্লক করেছে।"
        )
        try:
...
def main():
    ...
    print("Bot is starting with Original Modified Engine...")
